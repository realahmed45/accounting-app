  ...
  return (
    <div className="relative w-full sm:w-auto">
      <button
        ...
        className="flex items-center gap-2 sm:gap-3 px-3 sm:px-5 py-2.5 sm:py-3 bg-slate-700/50 backdrop-blur-sm border-2 border-slate-600 hover:border-slate-500 rounded-xl hover:bg-slate-700 transition-all min-w-[170px] sm:min-w-[220px] w-full sm:w-auto shadow-lg hover:shadow-xl"
      >
        <div className="bg-gradient-to-br from-blue-500 to-purple-600 p-2 rounded-lg">
          <Building2 className="w-4 h-4 sm:w-5 sm:h-5 text-white" />
        </div>
        <span className="flex-1 text-left font-bold text-white truncate text-sm sm:text-base">
          {currentAccount.accountName}
        </span>
        <ChevronDown
          className={`w-4 h-4 sm:w-5 sm:h-5 text-slate-300 transition-transform ${isOpen ? "rotate-180" : ""}`}
        />
      </button>
      ...
      {isOpen && (
        <>
          ...
          />
          <div className="absolute top-full left-0 right-0 mt-2 bg-white border-2 border-slate-200 rounded-xl shadow-2xl z-20 overflow-hidden animate-slideDown max-h-[70vh]">
