...
const ExtraHoursPanel = ({ accountId, currentMember }) => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-10 animate-in fade-in duration-1000">
      {/* Header Module */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white/5 backdrop-blur-3xl p-4 sm:p-10 rounded-2xl sm:rounded-[3rem] border border-white/10 shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-gray-400 font-bold uppercase tracking-widest text-xs italic">
            Overtime log audit & manager validation sequence
          </p>
        </div>
        <button
        ...
        {records.length === 0 ? (
          <div className="bg-white/5 py-24 rounded-[3rem] text-center border-2 border-dashed border-white/10">
            ...
            <p className="text-gray-500 font-black uppercase tracking-widest">
              Feed Empty — All Submissions Processed
            </p>
          </div>
        ) : (
          records.map((record) => (
            <div
              key={record._id}
              className="bg-white/5 backdrop-blur-3xl rounded-2xl sm:rounded-[2.5rem] p-4 sm:p-8 shadow-2xl border border-white/5 group hover:border-white/20 transition-all duration-500"
            >
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 sm:gap-10">
                <div className="flex items-center gap-4 sm:gap-8">
                  <div
                    className={`w-20 h-20 rounded-[1.5rem] flex flex-col items-center justify-center font-black relative overflow-hidden shadow-2xl border-2 ${
                  ...
                  >
                    <span className="text-2xl leading-none">
                      {Math.round((record.durationMinutes / 60) * 10) / 10}
                    </span>
                    <span className="text-[10px] mt-1 opacity-60 uppercase">
                      Hours
                    </span>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      <p className="text-lg font-black text-white">
                        {record.memberId?.displayName || "Member"}
                      </p>
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-[11px] font-black uppercase tracking-widest text-gray-500">
                      <span className="flex items-center gap-2 bg-white/5 px-3 py-1.5 rounded-full border border-white/5">
                        <Clock className="w-3.5 h-3.5" /> {record.startTime} —{" "}
                        {record.endTime}
                      </span>
                      {record.reason && (
                        <span className="flex items-center gap-2 text-indigo-400">
                          <Shapes className="w-3.5 h-3.5" /> {record.reason}
                        </span>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-6 border-t md:border-t-0 border-white/10 pt-8 md:pt-0">
                  <div className="flex items-center gap-2.5 mr-6">
                    <div
                      className="w-12 h-12 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-gray-400 group-hover:bg-indigo-500/20 group-hover:text-indigo-400 transition-all duration-500"
                      title="Visual Proof Captured"
                    >
                      <Camera className="w-6 h-6" />
                    </div>
                    <div
                      className="w-12 h-12 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-gray-400 group-hover:bg-indigo-500/20 group-hover:text-indigo-400 transition-all duration-500"
                      title="Neural GPS Verified"
                    >
                  ...
                  {record.status === "pending" && isManager ? (
                    ...
                    <div
                      className={`px-8 py-3 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] border shadow-2xl ${
                        record.status === "approved"
                          ...
                          : record.status === "rejected"
                            ...
                            : "bg-orange-500/10 border-orange-500/30 text-orange-400 shadow-orange-900/20"
                      }`}
                    >
                      {record.status}
                    </div>
        ...
        <div className="fixed inset-0 z-[150] bg-[#0a0a0f]/90 backdrop-blur-3xl flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="bg-[#1a1a2e] w-full max-w-lg rounded-[3.5rem] shadow-[0_64px_128px_-16px_rgba(0,0,0,0.8)] overflow-hidden border border-white/10 animate-in zoom-in-95 duration-500">
            <div className="p-4 sm:p-10 pb-0">
              ...
              <p className="text-indigo-400 text-xs font-black uppercase mt-2 tracking-[0.3em]">
            ...
            <form
              ...
              className="p-4 sm:p-10 space-y-6 sm:space-y-8"
            >
              <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                <div className="space-y-3">
                  ...
                  <input
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-5 text-sm font-black text-white appearance-none"
                  />
                </div>
                <div className="space-y-3">
                  <label className="text-[10px] font-black uppercase text-gray-500 tracking-[0.2em]">
