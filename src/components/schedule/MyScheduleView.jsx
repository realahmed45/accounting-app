...
const MyScheduleView = ({ accountId }) => {
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center py-40">
      ...
      <p className="text-gray-400 font-black uppercase tracking-[0.3em] text-[10px] mt-8">Fetching Personal Feed...</p>
    </div>
  );
    ...
    <div className="max-w-6xl mx-auto space-y-10 animate-in fade-in duration-1000">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <PremiumStatCard 
          ...
          value={myShifts[0] ? new Date(myShifts[0].date).toLocaleDateString(undefined, { day: 'numeric', month: 'short' }) : "Standby"} 
      ...
      <div className="bg-white/5 backdrop-blur-3xl rounded-[3rem] p-10 border border-white/10 shadow-2xl overflow-hidden relative">
        ...
        <div className="relative z-10 mb-12 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <h2 className="text-4xl font-black text-white tracking-tighter">Mission Roster</h2>
            <p className="text-gray-400 font-bold uppercase tracking-widest text-xs mt-2">Active Assigned Shifts</p>
          </div>
          <div className="bg-white/5 px-6 py-3 rounded-2xl border border-white/10 flex items-center gap-3">
             <span className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em]">Queue Capacity</span>
             <span className="text-xl font-black text-white">{myShifts.length}</span>
          </div>
        </div>
        ...
        {myShifts.length === 0 ? (
          <div className="text-center py-24 bg-white/5 rounded-[2.5rem] border border-dashed border-white/10">
            ...
            <p className="text-gray-400 font-black uppercase tracking-widest">System Idle — No Shifts Assigned</p>
          </div>
        ) : (
          <div className="space-y-6">
            {myShifts.map((shift) => {
              ...
              const label = shift.shiftTypeId ? shift.shiftTypeId.name : (shift.adHocLabel || "Custom Deployment");
              ...
              return (
                <div key={shift._id} className={`group relative p-8 rounded-[2.5rem] border-2 transition-all duration-500 hover:shadow-[0_0_40px_rgba(99,102,241,0.2)] ${
                  isToday ? "border-indigo-500/50 bg-indigo-500/5 shadow-2xl" : "border-white/5 bg-white/[0.02] hover:border-white/20"
                }`}>
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 relative z-10">
                    <div className="flex items-center gap-8">
                      <div className="relative">
                        {isToday && <div className="absolute inset-0 bg-indigo-500 blur-xl opacity-40 animate-pulse" />}
                        <div className="text-center p-4 rounded-3xl bg-white/5 backdrop-blur-md shadow-2xl border border-white/10 min-w-[100px] relative z-20">
                          <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">
                            {shiftDate.toLocaleDateString(undefined, { weekday: 'short' })}
                          </p>
                          <p className="text-3xl font-black text-white mt-1 leading-none">{shiftDate.getDate()}</p>
                        </div>
                      </div>

                      <div>
                        <div className="flex items-center gap-3">
                           <div className="w-3 h-3 rounded-full blur-[2px]" style={{ backgroundColor: color }} />
                           <h3 className="text-2xl font-black text-white tracking-tight">{label}</h3>
                        </div>
      ...
      <div className="bg-gradient-to-br from-[#1a1a2e] to-[#0f0f1a] rounded-[3rem] p-10 flex flex-col md:flex-row items-center justify-between gap-10 border border-white/10 shadow-3xl overflow-hidden relative">
        ...
        <div className="relative z-10 flex gap-6 items-start">
           ...
           <div>
              <h4 className="text-2xl font-black text-white tracking-tight">Geo-Verified Environment</h4>
              <p className="text-gray-400 font-medium mt-2 leading-relaxed max-w-lg">
                Automated protocols capture neural GPS coordinates and visual proof during check-in. This data is encrypted end-to-end to ensure operational integrity.
              </p>
           </div>
        </div>
        ...
        <div className="relative z-10 group cursor-pointer">
           ...
           <div className="bg-indigo-600/10 px-8 py-6 rounded-[2rem] border border-indigo-500/30 backdrop-blur-md relative z-10">
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-indigo-300 mb-3">Verification Badge</p>
...
const PremiumStatCard = ({ label, value, icon, gradient, sub }) => (
  <div className="group relative bg-[#1a1a2e] rounded-[2.5rem] p-1 border border-white/5 shadow-2xl transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.6)]">
    <div className={`absolute inset-0 bg-gradient-to-br ${gradient} opacity-0 group-hover:opacity-5 transition-opacity rounded-[2.5rem]`} />
    <div className="bg-[#0f0f1a] rounded-[2.3rem] p-8 h-full relative z-10 border border-white/5">
       <div className={`w-14 h-14 bg-gradient-to-br ${gradient} rounded-2xl flex items-center justify-center text-white shadow-3xl mb-6 transition-transform duration-500 group-hover:scale-110 group-hover:rotate-6`}>
         {icon}
       </div>
       <p className="text-[10px] font-black text-gray-500 uppercase tracking-[0.3em] mb-2">{label}</p>
       <h3 className="text-3xl font-black text-white tracking-tighter">{value}</h3>
       <p className="text-xs font-bold text-indigo-400 mt-4 opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-2 group-hover:translate-y-0 uppercase tracking-widest">{sub}</p>
    </div>
  </div>
);
