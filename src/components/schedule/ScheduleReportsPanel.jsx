...
const ScheduleReportsPanel = ({ accountId }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-12 animate-in fade-in duration-1000">
      {/* Analytics Header */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-white/5 backdrop-blur-3xl p-6 sm:p-12 rounded-2xl sm:rounded-[3.5rem] border border-white/10 shadow-3xl relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-center gap-4 sm:gap-8">
          ...
          <div>
            ...
            <p className="text-gray-400 font-bold uppercase tracking-widest text-[10px] mt-2 italic flex items-center gap-2">
              <Target className="w-3 h-3 text-indigo-400" /> Operational
              Efficiency & Distribution Audit
            </p>
          </div>
        </div>
      ...
      {!data || data.length === 0 ? (
        <div className="bg-white/5 py-40 rounded-[3.5rem] text-center border-2 border-dashed border-white/10">
          ...
          <p className="text-gray-500 font-black uppercase tracking-widest">
            No activity signatures detected for this period
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
          ...
          <div className="xl:col-span-3 space-y-6">
            {data.map((item, idx) => (
              <div
                key={idx}
                className="bg-white/5 backdrop-blur-3xl rounded-xl sm:rounded-[2.5rem] p-4 sm:p-10 border border-white/5 hover:border-white/20 transition-all duration-500 group relative overflow-hidden"
              >
                <div className="absolute top-0 right-0 p-10 bg-indigo-500/5 blur-[50px] rounded-full" />

                <div className="flex flex-col md:flex-row items-center justify-between gap-4 sm:gap-10 relative z-10">
                  <div className="flex items-center gap-8">
                    ...
                    <div>
                      <h4 className="text-2xl font-black text-white tracking-tight group-hover:text-indigo-300 transition-colors uppercase">
                        {item.member.displayName}
                      </h4>
                      <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest mt-1 inline-block">
                  ...
                  <div className="flex flex-wrap justify-center gap-10">
                    <MetricBox
                      label="Actual Hours"
                      value={Math.round(item.stats.totalHours * 10) / 10}
                      icon={<Clock className="w-4 h-4" />}
                      color="text-indigo-400"
                    />
                    <MetricBox
                      label="Leave Taken"
                      value={item.stats.timeOffDays}
                    ...
                    <MetricBox
                      label="Efficiency"
                      value={`${Math.round((item.stats.totalHours / 160) * 100)}%`}
                ...
                <div className="mt-8 pt-8 border-t border-white/5 flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="h-1.5 w-48 bg-white/5 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${Math.min((item.stats.totalHours / 160) * 100, 100)}%`,
                        }}
                      />
                    </div>
                  ...
                  <button className="text-[10px] font-black text-indigo-400 hover:text-white uppercase tracking-widest decoration-dotted underline underline-offset-4">
                    Full Matrix Audit
                  </button>
                </div>
              </div>
          ...
          <div className="space-y-8">
            <div className="bg-[#1a1a2e] rounded-2xl sm:rounded-[3rem] p-6 sm:p-10 border border-white/10 shadow-3xl text-center relative overflow-hidden">
              ...
              <h5 className="text-sm font-black text-gray-400 uppercase tracking-[0.3em] mb-2 relative z-10">
                Team Payload
              </h5>
            ...
            <div className="bg-gradient-to-br from-indigo-900 to-purple-900 rounded-2xl sm:rounded-[3rem] p-6 sm:p-10 text-white relative shadow-3xl overflow-hidden group">
              ...
              <h5 className="text-lg font-black tracking-tight mb-4 relative z-10 leading-tight">
                Collective Optimization Engine
              </h5>
              <p className="text-sm font-medium text-white/60 leading-relaxed mb-8 relative z-10">
                Automated protocols are analyzing work patterns to suggest
                dynamic roster adjustments based on historical throughput.
              </p>
              <button className="w-full py-4 bg-white text-indigo-900 font-bold rounded-2xl text-[10px] uppercase tracking-widest hover:bg-gray-100 transition-all relative z-10">
                Trigger Simulation
              </button>
            </div>
          </div>
...
const MetricBox = ({ label, value, icon, color, suffix = "Hrs" }) => (
  <div className="flex flex-col items-center">
    <div
      className={`w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center border border-white/5 mb-3 ${color} shadow-inner`}
    >
      {icon}
    </div>
    <div className="text-center">
      ...
      <p className="text-[9px] font-black text-gray-500 uppercase tracking-[0.2em] mt-2">
        {label}
      </p>
    </div>
  </div>
