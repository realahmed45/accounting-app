...
const ShiftTypeManager = ({ accountId }) => {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Confirm deletion of this deployment template?"))
      return;
    try {
  ...
  return (
    <div className="bg-white/5 backdrop-blur-3xl rounded-[3rem] border border-white/10 shadow-3xl overflow-hidden h-full flex flex-col animate-in fade-in duration-500">
      ...
      <div className="flex-1 p-4 sm:p-10 overflow-y-auto custom-scrollbar">
        {showAdd && (
          <form
            ...
            className="mb-6 sm:mb-10 p-4 sm:p-8 bg-white/5 rounded-xl sm:rounded-[2rem] border border-indigo-500/20 space-y-4 sm:space-y-6 animate-in slide-in-from-top-4 duration-300 shadow-inner"
          >
            <div className="space-y-3">
              <label className="text-[10px] font-black uppercase text-indigo-400 tracking-[0.2em] ml-2">
                Blueprint Name
              </label>
              <input
                required
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
              <div className="space-y-3">
                ...
                <input
                  type="time"
                  className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-sm font-black text-white appearance-none"
            ...
            <button
              type="submit"
              className="w-full py-5 bg-indigo-600 text-white font-black rounded-2xl shadow-3xl shadow-indigo-900 transition-all hover:bg-indigo-500 uppercase tracking-widest text-xs flex items-center justify-center gap-3"
            >
        ...
        <div className="space-y-4">
          {types.length === 0 ? (
            ...
            types.map((t) => (
              <div
                key={t._id}
                className="group flex items-center justify-between p-6 bg-white/[0.03] hover:bg-white/[0.07] rounded-3xl border border-white/5 transition-all duration-500 shadow-xl"
              >
                <div className="flex items-center gap-6">
                  <div
                    className="w-14 h-14 rounded-2xl flex flex-col items-center justify-center text-white shadow-2xl transition-transform duration-500 group-hover:scale-110 group-hover:-rotate-3 border border-white/10"
                    style={{ backgroundColor: t.color }}
                  >
                    <Clock className="w-6 h-6 opacity-60" />
                  </div>
                  <div>
                    <h5 className="text-lg font-black text-white tracking-tight group-hover:text-indigo-300 transition-colors uppercase">
                      {t.name}
                    </h5>
                    <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mt-1">
      ...
      <div className="p-4 sm:p-8 bg-black/20 flex items-center gap-4 border-t border-white/5">
        <div className="w-10 h-10 bg-indigo-500/10 rounded-xl flex items-center justify-center text-indigo-400">
          <Sparkles className="w-5 h-5 animate-pulse" />
        </div>
        <p className="text-[9px] font-black text-gray-500 uppercase tracking-widest leading-relaxed">
          System allows ad-hoc overrides, but blueprint utilization is
          recommended for operational consistency.
        </p>
      </div>
    </div>
  );
