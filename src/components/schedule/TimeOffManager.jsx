...
const TimeOffManager = ({ accountId }) => {
  ...
  const handleUpdate = async (memberId, updates) => {
    ...
    setSuccess("");
    try {
      await timeOffService.updateAllowance(accountId, memberId, {
        ...updates,
        year,
      });
      setSuccess(
        `Synced for ${members.find((m) => m._id === memberId)?.displayName}`,
      );
    ...
    <div className="bg-white/5 backdrop-blur-3xl rounded-[3rem] border border-white/10 shadow-3xl overflow-hidden h-full flex flex-col animate-in fade-in duration-500">
        ...
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mt-1">
              Annual Leave Analytics ({year})
            </p>
          </div>
        </div>
      </div>

      <div className="flex-1 p-4 sm:p-10 overflow-y-auto custom-scrollbar space-y-4 sm:space-y-8">
        {members.map((member) => {
          ...
          const remaining =
            ...
            balance.extraEarnedDays -
            balance.usedDays;

          return (
            <div
              key={member._id}
              className="group bg-white/[0.03] hover:bg-white/[0.07] rounded-2xl sm:rounded-[2.5rem] p-4 sm:p-8 border border-white/5 transition-all duration-500 shadow-xl relative overflow-hidden"
            >
              ...
              <div className="flex items-center gap-6 mb-8 relative z-10">
                ...
                <div>
                  <h5 className="text-xl font-black text-white tracking-tight uppercase">
                    {member.displayName}
                  </h5>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
                <div className="space-y-4">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setBalances((p) => ({
                          ...
                          [member._id]: {
                            ...balance,
                            annualAllowance: parseInt(e.target.value) || 0,
                          },
                        }))
                      }
                      onBlur={() =>
                  ...
                  <div className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5 shadow-inner">
                    ...
                    <input
                      ...
                      onBlur={() =>
                        handleUpdate(member._id, { usedDays: balance.usedDays })
                      }
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  ...
                  <div
                    className={`p-4 rounded-2xl border flex flex-col justify-between transition-all duration-500 shadow-2xl ${
                      remaining < 0
                        ? "bg-red-900/20 border-red-500/20 shadow-red-900/20"
                        : "bg-green-900/20 border-green-500/20 shadow-green-900/20"
                    }`}
                  >
                    <span
                      className={`text-[9px] font-black uppercase tracking-widest ${remaining < 0 ? "text-red-400" : "text-green-400"}`}
                    >
                      Remaining
                    </span>
                    <div className="mt-2 flex items-baseline gap-1">
                      <span
                        className={`text-2xl font-black ${remaining < 0 ? "text-red-400" : "text-green-400"}`}
                      >
                        {remaining}
                      </span>
      ...
      <div className="p-4 sm:p-8 bg-black/20 flex items-center gap-4 border-t border-white/5">
        ...
        <p className="text-[9px] font-black text-gray-500 uppercase tracking-widest leading-relaxed">
          Neural bonus days are auto-credited via approved overtime matrix
          synchronization.
        </p>
      </div>
    </div>
