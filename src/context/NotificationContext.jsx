...
export const NotificationProvider = ({ children }) => {
  const { user } = useAuth();
  const TOAST_AUTO_DISMISS_MS = 50000;
  const DESKTOP_NOTIFICATION_AUTO_CLOSE_MS = 50000;
    ...
    try {
      ...
      if (response?.success && response?.data?.notifications) {
        ...
        if (recentUnread.length > 0) {
          ...
          // Play sound when enabled in user preferences
          if (shouldPlaySound()) {
  ...
  const showDesktopNotification = (notification) => {
    ...
    try {
      ...
      // Auto-close after 50 seconds (except urgent)
  ...
  // Play notification sound with a persistent AudioContext for mobile/browser reliability
